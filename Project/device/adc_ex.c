...
void ADC_IRQHandler(void)
{
    ...
    for(i=0; i< ADC_CHANNEL_NUM;i++)
    {
        ...
        {
			uint32_t adc_val = (int16_t)(adc_channel_read_data((adc_channel_t)i)&0x0000FFFF);
            adc_env.callback[i](i, adc_val);
        }
    }
#endif
...
int16_t adc_battery_voltage_read_by_single_pin(adc_channel_t ch_p, adc_pga_gain_t gain, adc_callback_t cb, int sample_num)
{
   ...
   // if (cb != NULL) return 0;
	...
	if (cb != NULL)
	{
		cb((adc_data2float(channel_config.inp_gp, sum)*1000),sum);
	}
    return (int16_t)(adc_data2float(channel_config.inp_gp, sum)*1000);
}
/*
	??????????J????????????adc? ?????k???? sample_num????????J??? ???????
*/
int16_t adc_val_read_by_single_pin(adc_channel_t ch_p, adc_pga_gain_t gain,int sample_num)
{
    adc_channel_config_t channel_config;
    adc_config_t config;
    adc_init();
    ...
    channel_config.inp_gp   = ch_p;
    channel_config.inn_gp   = ADC_CHANNEL_CHIP_VCM;
    channel_config.clk_sel  = ADC_CLK_SEL_8MHZ;
    //channel_config.count    = sample_num;
    channel_config.gtune_gp = s_adc_gain_table[gain].gtune_gp;
    channel_config.scal_gp  = s_adc_gain_table[gain].scal_gp;
    channel_config.vcm_gp   = ADC_VCM_SEL_000mV;
...
/*
	?????????? ????adc??? ??????????ô???L??
*/
void adc_battery_voltage_read_by_dif_pin_unblock(adc_channel_t ch_p, adc_channel_t ch_n, adc_pga_gain_t gain, adc_callback_t cb, int sample_num)
{
    adc_channel_config_t channel_config;
    adc_config_t config;
    adc_init_irq();
...
void adc_battery_voltage_read_by_pin_unblock(adc_channel_t ch_p, adc_pga_gain_t gain, adc_callback_t cb, int sample_num)
{
    adc_channel_config_t channel_config;
    adc_config_t config;
    adc_init_irq();
...
/*
	??????????J????????????adc? ?????k???? sample_num????????J??? ?????
*/
int32_t adc_val_read_by_dif_pin(adc_channel_t ch_p, adc_channel_t ch_n, adc_pga_gain_t gain,int sample_num)
{
    adc_channel_config_t channel_config;
    adc_config_t config;
    adc_init();
