...
enum gapc_msg_id
{
    ...
    /// @see struct gapc_per_adv_sync_trans_cmd
    GAPC_PER_ADV_SYNC_TRANS_CMD            = TASK_FIRST_MSG(TASK_ID_GAPC) + 0x35,
#endif

#if (CONFIG_DIRECTION_FINDING_CONN_CTE)
    /* Constant Tone Extension */
    /// Constant Tone Extension Transmission configuration command
    GAPC_CTE_TX_CFG_CMD                    = TASK_FIRST_MSG(TASK_ID_GAPC) + 0x38,
    /// Constant Tone Extension Reception configuration command
    GAPC_CTE_RX_CFG_CMD                    = TASK_FIRST_MSG(TASK_ID_GAPC) + 0x39,
    /// Constant Tone Extension request control command (enable / disable)
    GAPC_CTE_REQ_CTRL_CMD                  = TASK_FIRST_MSG(TASK_ID_GAPC) + 0x3A,
    /// Constant Tone Extension Response control command (enable / disable)
    GAPC_CTE_RSP_CTRL_CMD                  = TASK_FIRST_MSG(TASK_ID_GAPC) + 0x3B,
    /// Indicate reception of a IQ Report event over a ble connection
    GAPC_CTE_IQ_REPORT_IND                 = TASK_FIRST_MSG(TASK_ID_GAPC) + 0x3C,
#endif
