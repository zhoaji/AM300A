...
enum gattc_msg_id
{
    ...
    GATTC_READ_DB_HASH_CMD = 0x20 + TASK_FIRST_MSG(TASK_ID_GATTC),
    ...
    GATTC_CON_INFO_IND = 0x28 + TASK_FIRST_MSG(TASK_ID_GATTC),

    /// Start unknown command procedure
    GATTC_UNKNOWN_CMD  = 0xFF + TASK_FIRST_MSG(TASK_ID_GATTC),
    #endif
};
...
enum gattc_operation
{
    ...
    /* Robust Cache                                     */
    /* ************************************************ */
    /// Read peer database hash, please use GATTC_READ_BY_UUID replace
    GATTC_READ_DB_HASH,
    /// Enable Robust database cache feature, please uase GATTC_WRITE replace
    GATTC_ROBUST_DB_CACHE_EN,
    #endif
};
...
#if (CONFIG_BLE_VERSION >= BLE_VERSION_51)
/// Parameters for @ref GATTC_CON_INFO_IND message
/*@TRACE*/
struct gattc_con_info_ind
{
    ...
    uint16_t chg_hdl;
    /// client bond data information
    uint8_t cli_info;
    /// Client supported features    (@see enum gapc_cli_feat)
    uint8_t cli_feat;
};

/// enable robust database caching
/*@TRACE*/
...
/// This also means that no more indication/notification will be received till database hash isn't read
/*@TRACE*/
struct gattc_db_cache_out_of_sync_ind
{
    uint8_t     rsv;
};
