...
#ifdef HOPPBH_REPORT_NOTIFY_SUPPORT
                    if ((params->cfg[svc_idx].report_char_cfg[report_idx] & HOGPD_CFG_REPORT_NI) == HOGPD_CFG_REPORT_NI)
                    {
...
#ifdef HOPPBH_REPORT_NOTIFY_SUPPORT
        uint8_t *att_value;
        att_size_t att_len = 0;
        if(attm_get_value(handle+1, &att_len, &att_value) == ATT_ERR_NO_ERROR)
        {
            if(att_value[1] == HOGPD_CFG_REPORT_FEAT &&(report->idx +1) == att_value[0])
            {
                status = GAP_ERR_NO_ERROR;
            }
        }
