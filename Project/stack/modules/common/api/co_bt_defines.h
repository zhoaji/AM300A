...
enum ble_feature
{
    ...
    BLE_CONN_CTE_REQ                   = (17),
    BLE_CONN_CTE_RSP                   = (18),
    BLE_CONNLESS_CTE_TX                = (19),
    BLE_CONNLESS_CTE_RX                = (20),
    BLE_ANT_SWITCH_DUR_CTE_TX          = (21),
    BLE_ANT_SWITCH_DUR_CTE_RX          = (22),
    BLE_RCV_CTE                        = (23),
    //byte 3
    BLE_PER_ADV_SYNC_TRANS_SND         = (24),
    BLE_PER_ADV_SYNC_TRANS_RCP         = (25),
    BLE_SLP_CLK_ACC_UPDATE             = (26),
...
/// Specify what PHY to use in test mode. HCI:7.8.78/79
enum le_phy_test
{
    //for Tx and Rx test PHY
    LE_TEST_PHY_1M          = 0x01,
    LE_TEST_PHY_2M          = 0x02,
    // for Tx test PHY
    LE_TEST_PHY_S8          = 0x03,
...
/*
 *  ************************************************************
 *              DIRECTION FINDING DEFINES
 * *************************************************************
 */
/// Minimum CTE Time in 8us units which can be set HCI 7.8.80
#define CTE_TIME_MIN                      (0x02)
/// Maximum CTE Time in 8us units which can be set HCI 7.8.80
#define CTE_TIME_MAX                      (0x14)

/// CTE type which can be set HCI 7.8.80
/*@TRACE*/
enum cte_type_value
{
    CTE_TYPE_AOA_VALUE          = 0,
    CTE_TYPE_AOD_1US_SLOT_VALUE = 1,
    CTE_TYPE_AOD_2US_SLOT_VALUE = 2,
    CTE_TYPE_VALUE_MAX          = CTE_TYPE_AOD_2US_SLOT_VALUE,
};

/// Specify what CTE Type Host prefers to use for HCI 7.8.84
enum cte_type_mask
{
    /// The Host prefers to allow AoA CTE Response (possibly among others)
    CTE_TYPE_AOA_BIT               = (1<<0),
    CTE_TYPE_AOA_POS               = (0),
    /// The Host prefers to allow AoD CTE Response with 1 us slots (possibly among others)
    CTE_TYPE_AOD_1US_SLOT_BIT      = (1<<1),
    CTE_TYPE_AOD_1US_SLOT_POS      = (1),
    /// The Host prefers to allow AoD CTE Response with 2 us slots (possibly among others)
    CTE_TYPE_AOD_2US_SLOT_BIT      = (1<<2),
    CTE_TYPE_AOD_2US_SLOT_POS      = (2),

    CTE_TYPE_ALL        = (CTE_TYPE_AOA_BIT | CTE_TYPE_AOD_1US_SLOT_BIT | CTE_TYPE_AOD_2US_SLOT_BIT),
};

#define CTE_TYPE_MAX                     (0x02)
/// Minimum number of CTE to transmit in each periodic advertising interval which can be set HCI 7.8.80
#define CTE_CNT_MIN                      (0x01)
/// Maximum number of CTE to transmit in each periodic advertising interval which can be set HCI 7.8.80
#define CTE_CNT_MAX                      (BLE_NB_TX_DESC_PER_ACT)
/// Minimum length of switch pattern which can be set HCI 7.8.80
#define CTE_SW_PTN_LEN_MIN               (0x02)
/// Maximum length of switch pattern which can be set HCI 7.8.80
#define CTE_SW_PTN_LEN_MAX               (0x4B)
/// Minimum length of IQ sample point which can be set HCI 7.7.65.21
#define CTE_IQ_SMP_MIN                   (0x09)
/// Maximum length of IQ sample point which can be set HCI 7.7.65.21
#define CTE_IQ_SMP_MAX                   (0x52)

/// CTE default parameters
/// default CTE Time
#define CTE_TIME_DEFAULT                 CTE_TIME_MIN
/// default cte type
#define CTE_TYPE_DEFAULT                 CTE_TYPE_AOA_VALUE
/// default cte count
#define CTE_CNT_DEFAULT                  CTE_CNT_MIN
/// default cte switch pattern length
#define CTE_SW_PTN_LEN_DEFAULT           CTE_SW_PTN_LEN_MIN

/// CTE IQ report packet status HCI 7.7.65.21, HCI 7.7.65.22
// The received period adv packet CRC was correct
#define CTE_PACKET_STATUS_CRC_CORRECT               0x00
// The received period adv packet CRC was incorrect and the Length and CTETime fields of the packet were used to determine sampling points
#define CTE_PACKET_STATUS_USING_CTETIME             0x01
// The received period adv packet CRC was incorrect but the Controller has determined the position and Length of CTETim
#define CTE_PACKET_STATUS_CNTL_DTRM                 0x02
// insufficient resources to sample
#define CTE_PACKET_STATUS_INVALID                   0xFF

/// CTE structure params
// CTE guard and reference period time (unit: us)
#define CTE_GRD_REF_PER_US                          (0x04 + 0x08)
// CTE sample interval when 1 us switch
#define CTE_SMP_INVL_1US_SW                         2
// CTE sample interval when 2 us switch
#define CTE_SMP_INVL_2US_SW                         4
// reference IQ sample count
#define CTE_REF_SMP_CNT                             8
// cte len to us
#define CTE_TIME_2_US(cte_time)                     ((cte_time) << 3)
/// CTE Time us caclculate from cte info including guard and reference period time
#define CTE_TIME_US_GET(cte_info)                   (GETF((cte_info), BLE_CTE_TIME) << 3)
/// CTE IQ sample cnt calculate
#define CTE_SMP_CNT_GET(cte_time_us, slt_dur)       ((((cte_time_us) - CTE_GRD_REF_PER_US) >> (slt_dur)) + CTE_REF_SMP_CNT)
/// LL_CTE_RSP PDU received successfully but without a CTE field, HCI 7.7.65.23
#define CTE_RSP_ERR_WO_CTE                         0x00

// define Ant ID max [0x00-0x03]
#define CTE_ANT_ID_MAX        0x03

/// CTE is transmiter & receiver selection
enum cte_tx_rx_select
{
    CTE_TRX_SEL_NONE    = 0x00,
    CTE_TRX_SEL_TX      = 0x01,
    CTE_TRX_SEL_RX      = 0x02,
    CTE_TRX_SEL_MAX,
};

#define CTE_INFO(cte_time, cte_type_val)      ((((cte_time) << BLE_CTE_TIME_LSB) & BLE_CTE_TIME_MASK) | (((cte_type_val) << BLE_CTE_TYPE_LSB) & BLE_CTE_TYPE_MASK))
...
/// Specify what mode Host prefers to use in Periodic Sync Transfer Mode. HCI:7.8.91 / HCI:7.8.92
enum le_per_sync_mode
{
    /// No attempt is made to synchronize to the periodic adverting.
    LE_PER_SYNC_MODE_ALL_DISABLE,
    /// An periodic sync receiced event is sent to Host, and HCI LE Periodic Adv Reports event will be disabled
    LE_PER_SYNC_MODE_PER_ADV_RCVD_ENABLE_ADV_REPORT_DISABLE,
    /// An periodic sync receiced event is sent to Host, and HCI LE Periodic Adv Reports event will be enabled
    LE_PER_SYNC_MODE_ALL_ENABLE,

    LE_PER_SYNC_MODE_MAX,
};

/// Specify waht cte type Host perfers to use in le sync. HCI:7.8.67 / HCI:7.8.91 / HCI:7.8.92
enum le_sync_cte_type_mask
{
    /// Do not sync to packets with AoA CTE
    LE_SYNC_CTE_TYPE_NOT_SYNC_AOA_BIT = (1<<0),
    LE_SYNC_CTE_TYPE_NOT_SYNC_AOA_POS = (0),
    /// Do not sync to packets with an AoD with 1 us slots
    LE_SYNC_CTE_TYPE_NOT_SYNC_AOD_1US_BIT = (1<<1),
    LE_SYNC_CTE_TYPE_NOT_SYNC_AOD_1US_POS = (1),
    /// Do not sync to packets with an AoD with 2 us slots
    LE_SYNC_CTE_TYPE_NOT_SYNC_AOD_2US_BIT = (1<<2),
    LE_SYNC_CTE_TYPE_NOT_SYNC_AOD_2US_POS = (2),
    /// Do not sync to packets with a type 3 CTE(reserved for future use)
    LE_SYNC_CTE_TYPE_NOT_SYNC_TYPE_3_BIT = (1<<3),
    LE_SYNC_CTE_TYPE_NOT_SYNC_TYPE_3_POS = (3),
    /// Do not sync to packets without a CTE
    LE_SYNC_CTE_TYPE_NOT_SYNC_WITHOUT_CTE_BIT = (1<<4),
    LE_SYNC_CTE_TYPE_NOT_SYNC_WITHOUT_CTE_POS = (4),
    /// sync all packets
    LE_SYNC_CTE_TYPE_SYNC_ALL = 0x00,
};
...
/// Size of supplement info(cte info) in extended header
...
/// CTE Info description
enum ble_cte_info
{
    // CTE TIME
    BLE_CTE_TIME_MASK = 0x1F,
    BLE_CTE_TIME_LSB  = 0,
    //CTE Type
    BLE_CTE_TYPE_MASK = 0xC0,
    BLE_CTE_TYPE_LSB  = 6,
};
...
/// SyncInfo - Offset threshold in us, below which Offset Units is set to 0
#define BLE_SYNC_OFFSET_UNIT_THRESHOLD         245700

/// SyncInfo - Maximum Packet Offset value(in us) that can be represented Sync Packet Offset
#define BLE_SYNC_PACKET_OFFSET_LIMIT           (((1<<13) - 1) * 300)

/// SyncInfo - Offset adjust threshold in us, below which Offset Adjust is set to 0.
#define BLE_SYNC_ADJUST_THRESHOLD              2457600

/// SyncInfo -Maximum offset value(in us) that can be represented.
#define BLE_SYNC_OFFSET_LIMIT          (BLE_SYNC_PACKET_OFFSET_LIMIT + BLE_SYNC_ADJUST_THRESHOLD)
...
/// SyncInfo - Offset Units description (1 bit)
#define BLE_SYNC_OFFSET_UNITS_MASK  0x2000
#define BLE_SYNC_OFFSET_UNITS_LSB   13

/// SyncInfo - Offset Adjust description (1 bit)
#define BLE_SYNC_OFFSET_ADJUST_MASK 0x4000
#define BLE_SYNC_OFFSET_ADJUST_LSB  14
...
/// LL_PERIODIC_SYNC sid(4 bits) atype(1 bit) sca(3 bits)
enum ble_per_sync_sid_atype_sca
{
    // SID
    BLE_PER_SYNC_SID_MASK    = 0x0F,
    BLE_PER_SYNC_SID_LSB     = 0,
    // ATYPE
    BLE_PER_SYNC_ATYPE_MASK  = 0x10,
    BLE_PER_SYNC_ATYPE_LSB   = 4,
    // SCA
    BLE_PER_SYNC_SCA_MASK    = 0xE0,
    BLE_PER_SYNC_SCA_LSB     = 5,
};
...
struct sync_info
{
    ...
    #endif  // (CONFIG_PER_ADV_SYNC_TRANSFER)
...
typedef struct
{
    ...
    // ****    Periodic Adv Sync Transfer Management     ****
    #if (CONFIG_PER_ADV_SYNC_TRANSFER)
    /// Default period adv sync transfer skip
    uint16_t   per_sync_skip;
    /// Default period adv sync transfer sync timeout
    uint16_t   per_sync_timeout;
    /// Default period adv sync transfer mode
    uint8_t    per_sync_mode;
    /// Default period adv sync transfer cte type (see enum le_sync_cte_type_mask)
    uint8_t    per_sync_cte_type_mask;
    #endif  //(CONFIG_PER_ADV_SYNC_TRANSFER)

} link_pref_t;

/// CTE IQ samples report
/*@TRACE*/
struct cte_smp_report
{
    uint8_t i_smp;
    uint8_t q_smp;
};
...
enum big_info_fields
{
    ...
    BIG_NUM_BIS_POS             = 0,
    BIG_NUM_BIS_LSB             = 27,
    BIG_NUM_BIS_MASK            = (int)0xF8000000,
    ...
    BIG_MAX_SDU_POS             = 16,
    BIG_MAX_SDU_LSB             = 20,
    BIG_MAX_SDU_MASK            = (int)0xFFF00000,
