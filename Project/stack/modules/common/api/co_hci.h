...
enum hci_opcode
{
    ...
    HCI_LE_SET_CONNLESS_CTE_TX_PARAM_CMD_OPCODE    = 0x2051,
    HCI_LE_SET_CONNLESS_CTE_TX_EN_CMD_OPCODE       = 0x2052,
    HCI_LE_SET_CONNLESS_IQ_SMP_EN_CMD_OPCODE       = 0x2053,
    HCI_LE_SET_CONN_CTE_RX_PARAM_CMD_OPCODE        = 0x2054,
    HCI_LE_SET_CONN_CTE_TX_PARAM_CMD_OPCODE        = 0x2055,
    HCI_LE_CONN_CTE_REQ_EN_CMD_OPCODE              = 0x2056,
    HCI_LE_CONN_CTE_RSP_EN_CMD_OPCODE              = 0x2057,
    HCI_LE_RD_ANT_INFO_CMD_OPCODE                  = 0x2058,
    HCI_LE_SET_PER_ADV_RX_EN_CMD_OPCODE            = 0x2059,
    HCI_LE_PER_ADV_SYNC_TSF_CMD_OPCODE             = 0x205A,
    HCI_LE_PER_ADV_SET_INFO_TSF_CMD_OPCODE         = 0x205B,
    HCI_LE_SET_PER_ADV_SYNC_TSF_PARAM_CMD_OPCODE   = 0x205C,
    HCI_LE_SET_DFT_PER_ADV_SYNC_TSF_PARAM_CMD_OPCODE = 0x205D,
    HCI_LE_GEN_DHKEY_V2_CMD_OPCODE                 = 0x205E,
    HCI_LE_MDF_SLP_CLK_ACCURACY_CMD_OPCODE         = 0x205F,
...
enum hci_le_evt_subcode
{
    ...
    HCI_LE_CONNLESS_IQ_REPORT_EVT_SUBCODE      = 0x15,
    HCI_LE_CONN_IQ_REPORT_EVT_SUBCODE          = 0x16,
    HCI_LE_CTE_REQ_FAIL_EVT_SUBCODE            = 0x17,
    HCI_LE_PER_ADV_SYNC_TRSF_RCVD_EVT_SUBCODE  = 0x18,
};
...
/*@TRACE
 * hci_le_gen_dhkey = hci_le_generate_dh_key_cmd*/
struct hci_le_generate_dh_key_v2_cmd
{
    /// Remote P-256 Public Key
    uint8_t public_key[64];
    /// key_type, 1:Use the debug private key
    uint8_t key_type;
};
...
struct hci_le_per_adv_create_sync_cmd
{
    ...
    uint16_t        sync_to;
    ///see enum le_sync_cte_type_mask
    uint8_t         sync_cte_type_mask;
};
...
/*@TRACE
 * hci_le_connless_iq_report_evt = hci_le_connless_iq_rep_evt*/
struct hci_le_connless_iq_rep_evt
{
    ///LE Subevent code
    uint8_t             subcode;
    /// Sync Handle to be used
    ...
    /// Packet status
    uint8_t             pkt_status;
    /// period adv event counter
    uint16_t            pa_evt_cnt;
    /// CTE sample count
    uint8_t             smp_cnt;
    /// CTE IQ samples
    struct cte_smp_report  iq_smp[CTE_IQ_SMP_MAX];
};

/*@TRACE
 * hci_le_conn_iq_report_evt = hci_le_conn_iq_rep_evt*/
struct hci_le_conn_iq_rep_evt
{
    ///LE Subevent code
    uint8_t             subcode;
    /// Connection Handle to be used
    uint16_t            conhdl;
    /// Rx PHY
    uint8_t             rxphy;
    /// Data channel index
...
/*@TRACE
 * hci_le_cte_report_evt = hci_le_cte_rep_evt*/
struct hci_le_cte_rep_fail_evt
{
    ///LE Subevent code
    uint8_t             subcode;
    /// status
...
struct hci_le_periodic_adv_sync_tsf_rcvd_evt
{
    ///LE Subevent code
    uint8_t             subcode;
    /// status
    ...
    uint16_t            per_adv_invl;
    /// advertiser clock accuracy
    uint8_t             adv_ca;
};

///HCI command complete event structure for LE Read Antenna Infomation command
/*@TRACE*/
struct hci_le_read_ant_info_cmd_cmp_evt
{
    /// Status of the command reception
    uint8_t    status;
    /// antenna switch sample rates that the local controller supported
    uint8_t    supported_sw_smp_rate;
    /// number of antennae that the local controller supported
    uint8_t    num_of_ant;
    /// Maximum length of switch pattern that the local controller supported
    uint8_t    max_len_sw_ptn;
    /// Maximum CTE length that the local controller supported
    uint8_t    max_cte_len;
};
...
/*@TRACE
 * hci_le_connless_iq_smp_en_cmp_evt = hci_le_connless_iq_sample_enable_cmp_evt*/
struct hci_le_connless_iq_sample_enable_cmp_evt
{
    /// Status of the command reception
    uint8_t     status;
    /// sync handle
...
/// HCI LE Receiver Test Command v3
/*@TRACE*/
struct hci_le_rx_test_v3_cmd
{
    ...
    uint8_t            sw_ptn_len;
    /// Antenna IDs
    uint8_t            ant_id[CTE_SW_PTN_LEN_MAX];
};

/// HCI LE Transmitter Test Command v3
/*@TRACE*/
...
///HCI LE Set Connectonless CTE Transmit Parameters structure
/*@TRACE*/
struct hci_le_set_connless_cte_tx_param_cmd
{
    /// Advertising handle
    uint8_t adv_hdl;
    /// CTE length
    ...
    /// CTE count
    uint8_t cte_cnt;
    /// length of switch pattern
    uint8_t sw_ptn_len;
    /// antenna_IDs
    uint8_t ant_id[CTE_SW_PTN_LEN_MAX];
};

///HCI LE Set Connectionless CTE Transmit Enable command structure
/*@TRACE*/
struct hci_le_set_connless_cte_tx_en_cmd
{
    /// Advertising handle
    uint8_t adv_hdl;
    /// CTE enable
...
///HCI LE Set Connectionless IQ Sampling Enable command structure
/*@TRACE*/
struct hci_le_set_connless_iq_smp_en_cmd
{
    /// sync handle
    uint16_t sync_hdl;
    /// sampleing enable
    uint8_t smp_en;
    /// switching and sampling slots
    uint8_t slt_dur;
    /// max sampled CTEs
    uint8_t max_smp_ctes;
    /// length of switch pattern
    uint8_t sw_ptn_len;
    /// antenna_IDs
    uint8_t ant_id[CTE_SW_PTN_LEN_MAX];
...
///HCI LE Set Connection CTE receive Parameters command structure
/*@TRACE*/
struct hci_le_set_conn_cte_rx_param_cmd
{
    /// connection handle
    uint16_t conhdl;
    /// sampleing enable
...
///HCI LE Set Connecton CTE transmit Parameters command structure
/*@TRACE*/
struct hci_le_set_conn_cte_tx_param_cmd
{
    /// connection handle
    uint16_t conhdl;
    /// CTE type
...
///HCI LE Connecton CTE request enable command structure
/*@TRACE*/
struct hci_le_conn_cte_req_en_cmd
{
    /// connection handle
    uint16_t conhdl;
    /// enable
    ...
    /// requested CTE length
    uint8_t req_cte_len;
    /// requested CTE type
    uint8_t req_cte_type;
};

///HCI LE Connecton CTE response enable command structure
/*@TRACE*/
struct hci_le_conn_cte_rsp_en_cmd
{
    /// connection handle
    uint16_t conhdl;
    /// enable
...
///HCI LE Set Periodic Advertising Receive Enable Command structure
/*@TRACE*/
struct hci_le_set_per_adv_rx_en_cmd
{
    /// Sync handle
    uint16_t sync_hdl;
    /// report enable
...
///HCI LE Periodic Advertising Sync Transfer Command structure
/*@TRACE*/
struct hci_le_per_adv_sync_tsf_cmd
{
    /// connection handle
    uint16_t conhdl;
    /// provided by Host to identify the periodic adv train to the peer device
    uint16_t service_data;
    /// Sync handle
    uint16_t sync_hdl;
};

///HCI LE Periodic Advertising Set Info Transfer Command structure
/*@TRACE*/
struct hci_le_per_adv_set_info_tsf_cmd
{
    /// connection handle
    uint16_t conhdl;
    /// provided by Host to identify the periodic adv train to he peer device
    uint16_t service_data;
    /// advertising handle
    uint8_t adv_hdl;
};

///HCI LE Periodic Advertising Sync Transfer Parameters Command structure
/*@TRACE*/
struct hci_le_set_per_adv_sync_tsf_param_cmd
{
    /// connection handle
    uint16_t conhdl;
    /// mode
    uint8_t mode;
    /// specifies skip the number of consecutive periodic adv packets
    uint16_t skip;
    /// Maximum permitted time between successful receives
    uint16_t sync_timeout;
    /// CTE type specifies whether to only synchronize to periodic adv with certain types of CTE (see enum le_sync_cte_type_mask)
    uint8_t sync_cte_type_mask;
};

///HCI LE Default Periodic Advertising Sync Transfer Parameters Command structure
/*@TRACE*/
struct hci_le_set_dft_per_adv_sync_tsf_param_cmd
{
    /// mode
    uint8_t mode;
    /// specifies skip the number of consecutive periodic adv packets
...
///HCI LE Modify Sleep Clock Accuracy Command structure
/*@TRACE*/
struct hci_le_mdf_slp_clk_accuracy_cmd
{
    /// action--switch to a more/less accurate clock
    uint8_t action;
};
