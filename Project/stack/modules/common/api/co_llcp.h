...
enum co_llcp_op_code
{
    ...
    LL_PERIODIC_SYNC_IND_OPCODE,
    /// Clock accuracy request
    LL_CLOCK_ACCURACY_REQ_OPCODE,
    /// Clock accuracy response
    LL_CLOCK_ACCURACY_RSP_OPCODE,
...
enum co_llcp_length
{
    ...
    LL_CTE_REQ_LEN                   = 2,
    LL_CTE_RSP_LEN                   = 1,
    LL_PERIODIC_SYNC_IND_LEN         = 35,
    LL_CLOCK_ACCURACY_REQ_LEN        = 2,
    LL_CLOCK_ACCURACY_RSP_LEN        = 2,
...
struct ll_cte_req
{
    /// op_code
    uint8_t op_code;
    ///cte info req; including miniimum length of the CTE, in 8us units and cte type
    uint8_t req_cte_info;
};

/// LL_CTE_RSP structure
/*@TRACE
 * @NO_PAD
*/
struct ll_cte_rsp
...
struct ll_cte_rsp_ext
{
    /// opcode
    uint8_t     op_code;

    ///The followings are extended for CTE process(user-defined).
    /// CTE info
    uint8_t     cte_info;
    /// CTE smple IQ data ptr;
    uint8_t*    cte_smp_data_ptr;
    /// rssi
    int16_t     rssi;
    /// data channel index
 ...
 * @NO_PAD
*/
struct  ll_periodic_sync_ind
{
    /// op_code
    uint8_t             op_code;
    /// identifier provided by Host
    uint16_t            id;

    /// periodic advertising sync info
    ///sync packet offset; offset units; offset adjust
    uint16_t            sync_info_offset;
    /// periodic advertising interval in 1.25 ms units.
    uint16_t            sync_info_invl;
    /// channel map on the physical channel; sleep clock accury
    struct le_chnl_map  sync_info_chmap_sca;
    /// periodic advertisng Access Address
    struct access_addr  sync_info_aa;
    /// periodic advertising crc init
    struct crc_init     sync_info_crc_init;
    /// contains the value of paEventCounter
    uint16_t            sync_info_evt_cnt;

    /// the periodic advertising train begin from
    uint16_t            conn_event_cnt;
    /// last period adv event count
    uint16_t            last_pa_event_cnt;
    /// Advertising SID subfield of the advertising set pointing to the periodic advertising; AdvA address; sleep clock accuracy
    uint8_t             sid_atype_sca;
    /// PHY(s) for periodic advertising
    uint8_t             phy;
    /// advertser's address
    struct bd_addr      adva;
    /// 
    uint16_t            sync_conn_event_cnt;
};

struct  ll_periodic_sync_ind_ext
{
    /// op_code
    uint8_t             op_code;
    /// identifier provided by Host
    ...
    ///The followings are extended for CTE process(user-defined)
    int16_t             sync_bit_off;
    uint32_t            anchor_ts;
};

/// LL_CLOCK_ACCURACY_REQ structure.
/*@TRACE
 * @NO_PAD
*/
struct ll_clk_accuracy_req
{
    /// opcode
    uint8_t             op_code;
    /// clock accuracy
...
/// LL_CLOCK_ACCURACY_RSP structure.
/*@TRACE
 * @NO_PAD
*/
struct ll_clk_accuracy_rsp
{
    /// opcode
    uint8_t             op_code;
    /// clock accuracy
...
union llcp_pdu
{
    ...
    //@trc_union op_code == LL_CTE_REQ_OPCODE
    struct ll_cte_req                   cte_req;

    //@trc_union op_code == LL_CTE_RSP_OPCODE
    struct ll_cte_rsp_ext               cte_rsp;

    //@trc_union op_code == LL_PERIODIC_SYNC_IND_OPCODE
    struct ll_periodic_sync_ind_ext     periodic_sync_ind;

    //@trc_union op_code == LL_CLOCK_ACCURACY_REQ_OPCODE
    struct ll_clk_accuracy_req          ca_req;

    //@trc_union op_code == LL_CLOCK_ACCURACY_RSP_OPCODE
    struct ll_clk_accuracy_rsp          ca_rsp;
